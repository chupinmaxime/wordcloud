...
\begin{remark}
  ...
  We first tried to compute intersections between words by decomposing the
...
\begin{remark}
The ``unity'' of weights is not really important because internally,
\package{wordcloud} compute new weights (with a min and a max, etc.) to work
...
\package{luamplib} to interpret a \hologo{METAPOST} code produced by Lua. 
...
should be possible to parse a \pdf{} with \hologo{LuaTeX}, though.} one can
